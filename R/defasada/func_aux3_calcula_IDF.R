...
IDF <- function(dados = banco_de_dados, lin_pextr = 249.5, lin_pobreza = 499, sal_min = 998, ano_inicial= "2016", dataref_dados = "2019-10-18", inpc){
  library(dplyr)
  library(tidyr)
  ## Sanity test - ENTRADA
  ...
  D[, h1 := fifelse(cod_especie_domic_fam==1 | cod_especie_domic_fam==2 | cod_especie_domic_fam==3,1,
                    fifelse(is.na(cod_especie_domic_fam),NA_real_,0))]
  
  # H2: DOMICILIO PARTICULAR PERMANENTE OU PROVISORIO
  D[, h2 := fifelse(cod_especie_domic_fam==1 | cod_especie_domic_fam==2,1,
                    fifelse(is.na(cod_especie_domic_fam),NA_real_,0))]
  
  # H3: DOMICILIO PARTICULAR PERMANENTE
  D[, h3 := fifelse(cod_especie_domic_fam==1, 1, fifelse(is.na(cod_especie_domic_fam),NA_real_,0))]
  
  # H4:DENSIDADE DE ATE 2 MORADORES POR DORMITORIO
  D[,h4 :=  fifelse(h3==1 & densidade<=2 & is.na(densidade)==FALSE, 1,
                    fifelse( (is.na(h3) & is.na(densidade) )| h3 != 1, NA_real_,0))]
  
  
  #H5: MATERIAL DE CONSTRU??O PERMANENTE EM DOMICILIO PARTICULAR PERMANENTE
  D[, h5 := fifelse(h3==1 & cod_material_domic_fam %in% c(1:4), 1,
                    fifelse((h3==1 & is.na(cod_material_domic_fam)) | h3 != 1,NA_real_,0))]
  
  # H6: ACESSO ADEQUADO A AGUA DE REDE GERAL DE DISTRIBUICAO EM DOMICILIO PARTICULAR PERMANENTE
  D[, h6 := fifelse(h3==1 & cod_abaste_agua_domic_fam==1, 1,
                    fifelse((h3==1 & is.na(cod_abaste_agua_domic_fam)) | h3 != 1,NA_real_,0))]
  
  # H7: ACESSO ADEQUADO A AGUA EM DOMICILIO PARTICULAR PERMANENTE
  D[, h7 :=  fifelse( (h3==1 & cod_abaste_agua_domic_fam==1) |(h3==1 & cod_abaste_agua_domic_fam==2) |
                        (h3==1 & cod_abaste_agua_domic_fam==3) , 1,
                      fifelse( (h3==1 & is.na(cod_abaste_agua_domic_fam) ) | h3 != 1,NA_real_,0))]  
  
  
  # H8: DOMICILIO PARTICULAR PERMANENTE POSSUI BANHEIRO OU SANITARIO
  D[, h8 := fifelse(h3==1 & cod_banheiro_domic_fam==1, 1,
                    fifelse(h3==1 & cod_banheiro_domic_fam==2, 0, NA_real_))]
  
  # H9: ESGOTAMENTO SANITARIO ADEQUADO EM DOMICILIO PARTICULAR PERMANENTE
  D[, h9 := fifelse(h3==1 & cod_banheiro_domic_fam==1 & cod_escoa_sanitario_domic_fam %in% c(1:2), 1,
                    fifelse(h3==1 & D$cod_banheiro_domic_fam==1 & is.na(D$cod_escoa_sanitario_domic_fam),NA_real_,
                            fifelse(h3!=1 | cod_banheiro_domic_fam!=1,NA_real_,0)))]
  
  # H10: LIXO COLETADO DE FORMA DIRETA EM DOMICILIO PARTICULAR PERMANENTE
  D[, h10 :=  fifelse(h3==1 & cod_destino_lixo_domic_fam==1, 1,
                      fifelse((h3==1 & is.na(cod_destino_lixo_domic_fam) ) | h3!=1, NA_real_,0))]
  
  # H11: LIXO COLETADO DE FORMA DIRETA OU INDIRETA EM DOMICILIO PARTICULAR PERMANENTE
  D[, h11 := fifelse((h3==1 & cod_destino_lixo_domic_fam %in% c(1:2)), 1,
                     fifelse((h3==1 & is.na(cod_destino_lixo_domic_fam)) | h3 != 1, NA_real_,0))]
  
  # H12: ACESSO A ELETRICIDADE COM MOTOR DE USO EM DOMICILIO PARTICULAR PERMANENTE
  D[, h12 := fifelse(h3==1 & cod_iluminacao_domic_fam %in% c(1:2), 1,
                     fifelse( (h3==1 & is.na(cod_iluminacao_domic_fam) ) | h3 != 1,NA_real_,0))]
  
  # H13: ACESSO A ELETRICIDADE EM DOMICILIO PARTICULAR PERMANENTE
  D[, h13 := fifelse((h3==1 & cod_iluminacao_domic_fam %in% c(1:3)), 1,
                     fifelse( ((h3==1 & is.na(cod_iluminacao_domic_fam) ) | h3 != 1),NA_real_,0))]
  
  # H14: Domicilio particular permanente localizado em trecho de logradouro com cacamba/pavimentacao total
  D[, h14 := fifelse(h3==1 & cod_calcamento_domic_fam==1, 1,
                     fifelse((h3==1 & is.na(cod_calcamento_domic_fam) ) | h3!=1, NA_real_, 0))]
  
  # H15: Domicilio particular permanente localizado em trecho de logradouro com cacamba/pavimentacao total ou parcial
  D[, h15 := fifelse(h3==1 & cod_calcamento_domic_fam %in% c(1,2), 1,
                     fifelse( (h3==1 & is.na(cod_calcamento_domic_fam) ) | h3!=1, NA_real_, 0))]
